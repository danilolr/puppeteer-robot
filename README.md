...
|---|---|---|---|
| **API** | `puppeter-robot-api/` | Backend REST API with WebSocket support | NestJS 11, Puppeteer Core, Socket.IO |
| **Frontend** | `puppeter-robot-ng/` | Web interface for managing automation tasks | Angular 20, Tailwind CSS, Socket.IO Client |
...
The frontend configuration is managed via `puppeter-robot-ng/docker/config.json`, which is mounted into the Nginx container at `/usr/share/nginx/html/assets/config.json`. Update this file to change the API URL or other frontend settings.
...
├── README.md
├── puppeter-robot-api/             # NestJS backend
│   ├── docker/
│   │   ├── Dockerfile.amd64
│   │   └── Dockerfile.arm64
│   ├── src/
│   └── package.json
└── puppeter-robot-ng/              # Angular frontend
    ├── docker/
    │   ├── Dockerfile
