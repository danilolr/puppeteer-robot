...
export class StartPage implements OnInit {
  ...
  async createNewRobot() {
    try {
      ...
      await api.robotControllerCreate({pool: poolNameValue === "" ? "none" : poolNameValue})
  ...
  async copyToClipboard(text: string) {
    try {
      ...
      this.showToastMessage('ID copiado para a área de transferência!')
    } catch (error) {
      console.error('Failed to copy to clipboard:', error)
      this.showToastMessage('Erro ao copiar ID')
    }
  }
